function [data, attribute, dimensions] = loadnc(nc_file, names, is_single);
...
for ii = 1:length(names)
  if any(strcmp(names{ii},allnames))
    ...
    if xtype == netcdf.getConstant('NC_FLOAT') | ~isempty(add_offset) | ~isempty(scale_factor)
      if is_single
	missing_value = single(get_missing_value(ncid, varid));
	data.(newname) = single(netcdf.getVar(ncid, varid));
      else
    ...
    if ~isempty(scale_factor)
      if is_single
	data.(newname) = single(data.(newname)) .* double(scale_factor);
      else
	data.(newname) = double(data.(newname)) .* double(scale_factor);
      end
    else
       scale_factor = 1.0;
    end
    if ~isempty(add_offset)
      if is_single
	data.(newname) = single(data.(newname)) + single(add_offset);
      else
	data.(newname) = double(data.(newname)) + double(add_offset);
      end
    else
       add_offset = 0.0;
    end
